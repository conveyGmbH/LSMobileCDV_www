...
(function () {
    ...
    ], function copyInit(_Global, _Base, _BaseUtils) {
        /**
            ...
            if (typeof defaultObj === "object") {
                ...
                for (var key in defaultObj) {
                    if (defaultObj.hasOwnProperty(key)) {
                        ...
                        } else if (typeof targetObj[key] === "object" && typeof defaultObj[key] === "object") {
                            copyMissingMembersByValue(targetObj[key], defaultObj[key]);
                        }
                    }
                }
            }
    ...
    WinJS.Namespace.define("AppData", {
        ...
        getErrorMsgFromResponse: function (errorResponse) {
            // useless HTML element names to remove:
            var ignoreElements = [ "meta", "title", "script", "style" ];
            var errorMsg = "";
            if (errorResponse) {
                if (typeof errorResponse === "string") {
                    ...
                    if (!errorResponse.data && (errorResponse.responseText || errorResponse.response)) {
                        try {
                            ...
                            var htmlDoc = document.implementation.createHTMLDocument("errorTextFrag");
                            WinJS.Utilities.setInnerHTMLUnsafe(htmlDoc.documentElement, (errorResponse.responseText || errorResponse.response));
                            var imported = document.importNode(htmlDoc.body, true);
                            while (imported && imported.childNodes && imported.childNodes.length > 0) {
                                var firstChild = imported.childNodes[0];
                                var nodeName = firstChild.nodeName;
                                if (!nodeName ||
                                    ignoreElements.indexOf(nodeName.toLowerCase()) < 0 &&
                                    firstChild.textContent) {
                                    var textContent = firstChild.textContent;
                                    while (textContent && textContent.charCodeAt(0) < 49) {
                                ...
                                imported.removeChild(firstChild);
                            }
                            errorResponse.data = {
                                error: {
                                    message: {
        ...
        setErrorMsg: function (bindingObject, errorResponse) {
            if (bindingObject) {
                ...
                if (error.errorMsg === "" || error.errorMsg === "Canceled") {
                    error = {
                        errorMsg: "",
                        displayErrorMsg: "none"
                    }
