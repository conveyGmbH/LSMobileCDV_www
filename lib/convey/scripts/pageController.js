                    ...
                    var ret = WinJS.Resources.processAll(that.element).then(function () {
                        ...
                        if (typeof device === "object") {
                                    ...
                                    AppBar.barControl._handleShowingKeyboardNew = function (event) {
                                        ...
                                        return ret.then(function () {
                                            if (occludedRect && occludedRect.height > 0) {
                                                var contentarea = Application.navigator.pageElement.querySelector(".contentarea");
                                                if (contentarea && contentarea.style) {
                                                    var newHeight = contentarea.clientHeight - occludedRect.height;
                                            ...
                                            if (Application.navigator.pageElement) {
                                                ...
                                                if (contentarea && contentarea.style) {
                                                    ...
                                                    if (AppBar.commandList &&
                                                        ...
                                                        var positionContent = WinJS.Utilities._getPositionRelativeTo(contentarea, null);
                            ...
                            if (device.platform === "windows" || device.platform === "Android") {
                                function addScrollIntoViewCheck(input) {
                                    if (input) {
                                        ...
                                        input.onfocus = function (event) {
                                            input._scrollIntoViewCheck = function (repeat) {
                                                ...
                                                if (activeElement === input &&
                                                    Application.navigator.pageElement.contains(that.element)) {
                                                    var contentarea = Application.navigator.pageElement.querySelector(".contentarea");
                                                    if (contentarea) {
                                                        function scrollIntoView() {
                                                            var position = WinJS.Utilities._getPositionRelativeTo(input, contentarea);
                                                            var top = position.top - contentarea.scrollTop;
                                                            var height = position.height;
                                                            var clientHeight = contentarea.clientHeight;
                                                            if (top < 20) {
                                                                Log.print(Log.l.trace, "scrollIntoView: scrollTop " + contentarea.scrollTop + "=>" + (contentarea.scrollTop - (20 - top)).toString());
                                                                contentarea.scrollTop -= 20 - top;
                                                            } else if ((top + height) > clientHeight) {
                                                                Log.print(Log.l.trace, "scrollIntoView: scrollTop " + contentarea.scrollTop + "=>" + (contentarea.scrollTop + (top + height - clientHeight)).toString());
                                                                contentarea.scrollTop += top + height - clientHeight;
                                                            } else {
                                                                Log.print(Log.l.trace, "scrollIntoView: top=" + top + " clientHeight=" + clientHeight + " scrollTop=" + contentarea.scrollTop);
                                                            }
                                                        }
                                                        function resizeView() {
                                                            var clientHeight = contentarea.clientHeight;
                                            ...
                                            if (target === input && typeof input._scrollIntoViewCheck === "function") {
                                ...
                                if (inputs && inputs.length > 0) {
                                    for (var i = 0; i < inputs.length; i++) {
                                        addScrollIntoViewCheck(inputs[i]);
                                    }
                                }
                            }
                        }
