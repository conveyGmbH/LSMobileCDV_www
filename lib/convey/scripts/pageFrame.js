        ...
        PageFrame: WinJS.Class.define(
            ...
            function PageFrame(aName) {
                ...
                dummyRequest: function () {
                    ...
                    Log.print(Log.l.trace, "calling metadata... url=" + options.url);
                    WinJS.xhr(options).then(function xhrSuccess(response) {
                        Log.print(Log.l.info, "dummy Test xhr metadata: method=GET url=" + options.url);
                        try {
                            var result = response.responseText;
                            Log.print(Log.l.info, "dummy Test xhr metadata: result=" + result);
                            return WinJS.Promise.as();
                        } catch (exception) {
                            Log.print(Log.l.error, "dummy Test xhr metadata: resource parse error " + (exception && exception.message));
                        }
                    }, function (errorResponse) {
                        Log.print(Log.l.error, "dummy Test xhr metadata: method=GET url=" + options.url + " returned error=" + AppData.getErrorMsgFromResponse(errorResponse));
                ...
                onResume: function () {
                    Log.call(Log.l.trace, "PageFrame.", "");
                    if (Application.pageframe) {
                        Application.pageframe.dummyRequest();
                    }
                ...
                checkForLanguage: function () {
                    Log.call(Log.l.trace, "PageFrame.", "");
                    var language;
                    //by Hung: IGNORE from persistentStates, always use language from device
