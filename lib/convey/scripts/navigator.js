        ...
        PageControlNavigator: WinJS.Class.define(
            ...
            function PageControlNavigator(element, options) {
                ...
                resizePageElement: function (element) {
                    ...
                    WinJS.Promise.timeout(0).then(function() {
                        // rearrange default button position
                ...
                _beforeload: function (args) {
                    var id = args.detail && args.detail.element && args.detail.element.id;
                    ...
                    args.detail.setPromise(fragment._beforeLoadPromise);
                    Log.ret(Log.l.trace, "");
                ...
                _beforenavigate: function (args) {
                    ...
                    if (isDisabled) {
                        ...
                        if (keys && keys.length > 0) {
                            ...
                            var doneFragments = 0;
                            var countFragments = keys.length;
                            var addFragmentPromise = function (fragmentId, fragment) {
                                if (fragment && fragment._element) {
                                    var detail = {
                                        element: fragment._element,
                                        setPromise: function (promise) {
                                            if (!promise) {
                                                promise = WinJS.Promise.as();
                                            }
                                            return promise.then(function (response) {
                                                doneFragments++;
                                                Log.print(Log.l.trace, "_beforeload success! doneFragments=" + doneFragments + "(countFragments=" + countFragments + ")");
                                                if (doneFragments === countFragments) {
                                                    checkCanUnload();
                                                }
                                            }, function (errorResponse) {
                                                ...
                                                WinJS.Promise.timeout(0).then(function () {
                                                    if (that._inBeforeNavigatePromise &&
                                                        ...
                                                        Log.print(Log.l.trace,
                                                            "from PageControlNavigator (false): _beforenavigate calling cancel()");
                                                        that._beforeNavigatePromise.cancel();
                                                        //that._beforeNavigatePromise = null;
                                                        that._inBeforeNavigatePromise = false;
                                                        if (typeof that._syncNavigationBar === "function") {
                                    ...
                                    var fragmentPromise = that._beforeload({ detail: detail });
                                    if (fragmentPromise) {
                                        fragmentPromises[fragmentId] = fragmentPromise;
                                    }
                                } else {
                                    doneFragments++;
                                }
                            ...
                            for (var j = 0; j < countFragments; j++) {
                                var id = keys[j];
                                addFragmentPromise(id, that._fragments[id]);
                            }
                            this._beforeNavigatePromise = new WinJS.Promise.as().then(function () {
