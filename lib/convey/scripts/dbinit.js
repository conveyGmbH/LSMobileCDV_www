...
(function () {
    ...
    WinJS.Namespace.define("AppData", {
        ...
        openDB: function (complete, error, progress, bReInit) {
            ...
            var psName = AppData._persistentStates.encodeSettings
                ? Application.pageframe.filenamePSEncoded
                : Application.pageframe.filenamePersistentStates;
            var doInit = function() {
            ...
            if (bReInit && cordova.file) {
                ...
                var fileNamePs = cordova.file.dataDirectory + psName;
                var currentTime = Date.now();
                window.resolveLocalFileSystemURL(cordova.file.dataDirectory, function (dirEntry) {
                    if (dirEntry) {
                        var copyDbToStorage = function(success) {
                            if (dataDirectory !== cordova.file.dataDirectory &&
                                DbCopyPlugin && typeof DbCopyPlugin.copyDbToStorage === "function") {
                                DbCopyPlugin.copyDbToStorage({
                                    fileName: dbName,
                                    fullPath: cordova.file.dataDirectory,
                                    overwrite: true
                                }).then(function () {
                                    var srcNameDb = cordova.file.dataDirectory + dbName;
                                    Log.print(Log.l.info, "Backup file success: " + srcNameDb);
                                    window.resolveLocalFileSystemURL(srcNameDb, function (fileEntryDb) {
                                        if (fileEntryDb) {
                                            var destNameDb = "Backup_" + currentTime + "_" + dbName;
                                            fileEntryDb.moveTo(dirEntry, destNameDb, function () {
                                                Log.print(Log.l.info, "Backup file success: " + destNameDb);
                                                success();
                                            }, function (error) {
                                                Log.print(Log.l.error, "Error moving file: ", error);
                                                doInit();
                                            });
                                        } else {
                                    ...
                                    Log.print(Log.l.error, "copyDbToStorage returned error: ", error);
                                    doInit();
                                });
                            } else {
                                window.resolveLocalFileSystemURL(fileNameDb, function (fileEntryDb) {
                                    if (fileEntryDb) {
                                        var destNameDb = "Backup_" + currentTime + "_" + dbName;
                                        fileEntryDb.copyTo(dirEntry, destNameDb, function() {
                                            Log.print(Log.l.info, "Backup file success: " + destNameDb);
                                            success();
                                        }, function (error) {
                        ...
                        copyDbToStorage(function() {
                            window.resolveLocalFileSystemURL(fileNamePs, function (fileEntryPs) {
                                if (fileEntryPs) {
                                    var destNamePs = "Backup_" + currentTime + "_" + psName;
                                    fileEntryPs.copyTo(dirEntry, destNamePs, function () {
