    ...
    WinJS.Namespace.define("AppData", {
        ...
        DbInit: WinJS.Class.define(function dbInit(complete, error, progress, bReInit) {
            ...
            constraintFromAttribSpec: function(that, attribName, attribFormat, attribFunction, attribParam, attribPParam, relationName) {
                ...
                if (attribFunction & 4096 && !(attribFunction & 16384)) {
                    if (attribPParam) {
                        var refTable = null, refColumn = null;
                        var posColumn = attribPParam.lastIndexOf(".");
                        if (posColumn >= 0) {
                ...
                var addScriptStmt = function(relationScriptRow) {
                    if (relationScriptRow.Status === 3 || // fertig
                        ...
                        if (relationName) {
                            ...
                            case 16: // UniqueIndex 
                            case 512: // Index 
                                stmt = "DROP INDEX IF EXISTS \"" + relationScriptRow.Title + "\"";
                                Log.print(Log.l.info, "createProjectScripts: " + stmt);
                                createScript.push(stmt);
                                stmt = "CREATE ";
                                if (relationScriptRow.ScriptTypeID === 16) {
                                    stmt += "UNIQUE ";
                                }
                                stmt += "INDEX \"" + relationScriptRow.Title + "\" ON \"" + relationName + "\"(" +
                                    relationScriptRow.Script + ")";
                                Log.print(Log.l.info, "createProjectScripts: " + stmt);
                                createScript.push(stmt);
                                break;
                            }
                        ...
                        if (loadCancelled) {
                            ...
                            relation.selectAll(function(json) {
                                ...
                                if (json && json.d && json.d.results && json.d.results.length > 0) {
                                    ...
                                    Log.print(Log.l.info, "loadProjectData.dbInsert: NO DATA for select LOAD" + relationName);
                                    var projectRelationIndex = projectRelations.indexOf(relation.relationName.substr(4));
                                    if (projectRelationIndex >= 0) {
                                        setRelationState(projectRelationIndex, "finished");
                    ...
                    var localDbInsert = function(relation, results) {
                        if (loadCancelled) {
                            ...
                            if (index >= 0) {
                                ...
                                } else if (getRelationState(index) !== "finished" && getRelationState(index) !== "loaded") {
                                ...
                                relation.dbInsert(relation, function() {
                                    ...
                                    if (!isRinf) {
                                        ...
                                        if (relation.relationName.substr(0, 4) !== "INIT" &&
                                            ...
                                            relation.relationName !== "MobileVersionSpec") {
                                            setRelationState(curindex, "loaded");
		...
		getDefLanguages: function () {
		    var defLanguages;
		    if (AppData._defLanguages && 
                typeof AppData._defLanguages.filter === "function" &&
                AppData._persistentStates.knownLanguageIds && 
                AppData._persistentStates.knownLanguageIds.length > 0) {
		        defLanguages = [];
		        for (var i = 0; i < AppData._persistentStates.knownLanguageIds.length; i++) {
		            var curLanguageId = AppData._persistentStates.knownLanguageIds[i];
		            var foundLanguage = AppData._defLanguages.filter(function (curLanguage) {
		                return curLanguage.LanguageSpecID === curLanguageId;
		            });
		            if (foundLanguage && foundLanguage.length > 0) {
		                defLanguages.push(foundLanguage[0]);
                    }
		        }
		    } else {
		        defLanguages = AppData._defLanguages;
		    }
		    return defLanguages;
		},
        ...
        openDB: function (complete, error, progress, bReInit) {
            var ret;
            Log.call(Log.l.trace, "AppData.");
            var dbName = AppData.getDbFileName();
            var doInit = function() {
            ...
            if (bReInit && cordova.file) {
                ...
                var fileNameDb = dataDirectory + dbName;
                var fileNamePs = cordova.file.dataDirectory +
                    (Application.pageframe.filenamePSEncoded || Application.pageframe.filenamePersistentStates);
                var currentTime = Date.now();
                window.resolveLocalFileSystemURL(cordova.file.dataDirectory, function (dirEntry) {
                    if (dirEntry) {
                        var copyDbFile = function(srcDirEntry) {
                            if (srcDirEntry) {
                                srcDirEntry.getFile(dbName, { create: false, exclusive: false }, function (fileEntryDb) {
                                    if (fileEntryDb) {
                                        var destNameDb = "Backup_" + currentTime + "_" + dbName;
                                        fileEntryDb.copyTo(dirEntry, destNameDb, function (newFileEntryDb) {
                                            Log.print(Log.l.info, "Backup file success: " + destNameDb);
                                            window.resolveLocalFileSystemURL(fileNamePs, function (fileEntryPs) {
                                                if (fileEntryPs) {
                                                    var destNamePs = "Backup_" + currentTime + "_" +
                                                        (Application.pageframe.filenamePSEncoded || Application.pageframe.filenamePersistentStates);
                                                    fileEntryPs.copyTo(dirEntry, destNamePs, function (newFileEntryPs) {
                                                        Log.print(Log.l.info, "Backup file success: " + destNamePs);
                                                        doInit();
                                                    }, function (error) {
                                                        Log.print(Log.l.error, "Error copying file: ", error);
                                                        doInit();
                                                    });
                                                } else {
                                                    Log.print(Log.l.error, "Error no FileEntry for: ", fileNamePs);
                                                    doInit();
                                                }
                                            }, function (error) {
                                                Log.print(Log.l.error, "Error resolving source file: ", error);
                                                doInit();
                                            });
                                        }, function (error) {
                                            Log.print(Log.l.error, "Error copying file: ", error);
                                        ...
                                        Log.print(Log.l.error, "Error no FileEntry for: ", fileNameDb);
                                        doInit();
                                    }
                                });
                            } else {
                                Log.print(Log.l.error, "Error no srcDirEntry for: ", dataDirectory);
                                doInit();
                            }
                        }
                        if (dataDirectory.indexOf("/databases/") === dataDirectory.length - 11) {
                            var dataParentDirectory = dataDirectory.substr(0, dataDirectory.length - 10);
                            window.resolveLocalFileSystemURL(dataParentDirectory, function (srcDirParentEntry) {
                                if (srcDirParentEntry) {
                                    srcDirParentEntry.getDirectory("databases/", { create: false, exclusive: false }, function (srcDirEntry) {
                                        copyDbFile(srcDirEntry);
                                    }, function (error) {
                                        Log.print(Log.l.error, "Error getDirectory: databases/", error);
                                        doInit();
                                    });
                                } else {
                                    Log.print(Log.l.error, "Error no srcDirParentEntry for: ", dataParentDirectory);
                                    doInit();
                                }
                            ...
                            window.resolveLocalFileSystemURL(dataDirectory, function (srcDirEntry) {
                                copyDbFile(srcDirEntry);
                            }, function (error) {
                                Log.print(Log.l.error, "Error resolving source file: ", error);
                                doInit();
                        ...
                        Log.print(Log.l.error, "Error no DireEntry for: ", dataDirectory);
                        doInit();
                    }
                }, function (error) {
                    Log.print(Log.l.error, "Error resolving destination directory: ", error);
                    doInit();
                });
            } else {
                doInit();
