    ...
    WinJS.Namespace.define(namespace, {
        Controller: WinJS.Class.derive(Application.Controller, function Controller(pageElement, commandList) {
                ...
                if (that.loadNextCount > 0) {
                    if (that.refreshDocPromise || that.refreshNextDocPromise) {
                        ...
                        that.refreshNextDocPromise = WinJS.Promise.timeout(250).then(function () {
                            ...
                            return ListRemote.contactDocView.selectNext(function (json) {
                        ...
                        if (listControl.selection) {
                            ...
                            if (selectionCount === 1) {
                                ...
                                listControl.selection.getItems().done(function (items) {
                                    var item = items[0];
                                    if (item.data && item.data.KontaktVIEWID && item.data.KontaktVIEWID === AppData.getRecordId("Kontakt_Remote")) {
                                        var contactPage;
                                        AppData.setRecordId("Kontakt_Remote", item.data.KontaktVIEWID);
                                        if (item.data.CreatorSiteID === AppData._persistentStates.odata.dbSiteId) {
                                            AppData.setRecordId("Kontakt", item.data.CreatorRecID);
                                            contactPage = "contact";
                                        } else {
            ...
            if (!restriction.importFilter) {
                restriction.MitarbeiterID = AppData.getRecordId("Mitarbeiter");
            } else {
                ...
                var ret = new WinJS.Promise.as().then(function () {
                    ...
                    var contactSelectPromise = ListRemote.contactView.select(function (json) {
                        ...
                        if (json && json.d && json.d.results && json.d.results.length > 0) {
                            ...
                            if (AppData.getRecordId("Kontakt_Remote")) {
                                that.binding.contactId = AppData.getRecordId("Kontakt_Remote");
                            }
                        } else {
                    ...
                    that.refreshDocPromise = WinJS.Promise.timeout(250).then(function () {
                        return ListRemote.contactDocView.select(function (json) {
                            // this callback will be called asynchronously
                            // when the response is available
                            Log.print(Log.l.trace, namespace + ".contactDocView: success!");
                            // startContact returns object already parsed from json file in response
            ...
            var loadNextUrl = function (recordId) {
                ...
                if (that.contacts && that.nextUrl && listView &&
                    ...
                    that.nextUrl = null;
                    that.refreshNextPromise = ListRemote.contactView.selectNext(function (json) { 
                        // this callback will be called asynchronously
                        // when the response is available
